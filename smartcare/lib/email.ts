  ...
  return {
    ...
    html: layout(`
      ...
      <p style="color:#475569;font-size:14px;margin:0 0 20px;">
        Hello ${escapeHtml(data.patientName)}, your appointment has been booked successfully.
      </p>
...
export function appointmentReminderEmail(data: {
  ...
  return {
    ...
    html: layout(`
      ...
      <p style="color:#475569;font-size:14px;margin:0 0 20px;">
        Hello ${escapeHtml(data.patientName)}, your appointment is <strong>tomorrow</strong>.
      </p>
...
export function appointmentCancelledEmail(data: {
  ...
  return {
    ...
    html: layout(`
      ...
      <p style="color:#475569;font-size:14px;margin:0 0 20px;">
        Hello ${escapeHtml(data.patientName)}, your appointment on
        <strong>${escapeHtml(data.date)} at ${escapeHtml(data.time)}</strong> has been cancelled.
      </p>
      <p style="font-size:13px;color:#64748b;margin:0;">
...
export function otpEmail(data: {
  ...
  return {
    ...
    html: layout(`
      ...
      <p style="color:#475569;font-size:14px;margin:0 0 20px;">
        Hello ${escapeHtml(data.name)}, use the code below to verify your identity.
      </p>
...
export function prescriptionEmail(data: {
  ...
  return {
    ...
    html: layout(`
      ...
      <p style="color:#475569;font-size:14px;margin:0 0 20px;">
        Hello ${escapeHtml(data.patientName)}, your prescription dated <strong>${escapeHtml(data.date)}</strong>.
      </p>
...
export function welcomeEmail(data: {
  ...
  return {
    ...
    html: layout(`
      ...
      <p style="color:#475569;font-size:14px;margin:0 0 20px;">
        Hello ${escapeHtml(data.name)}, your account has been created successfully.
      </p>
...
export function riskAlertEmail(data: {
  ...
  const factorsHtml = data.riskFactors
    .map(
      (f) => `<li style="margin-bottom:4px;font-size:13px;color:#334155;">${escapeHtml(f)}</li>`
    )
    .join("");
  ...
  return {
    ...
    html: layout(`
      <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:14px 18px;margin-bottom:20px;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <div>
            ...
            <div style="font-size:16px;font-weight:600;color:#0f172a;">${escapeHtml(data.patientName)}</div>
          </div>
      ...
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:12px 14px;">
        ...
        <div style="font-size:13px;color:#0f172a;">${escapeHtml(data.recommendation)}</div>
      </div>
    `),
  };
}
...
export function clinicSummaryEmail(data: {
  ...
  const listHtml = (items: string[], color: string) =>
    items.map((i) =>
      `<li style="margin-bottom:6px;font-size:13px;color:${color};padding-left:4px;">${escapeHtml(i)}</li>`
    ).join("");
  ...
  return {
    ...
    html: layout(`
      ...
      <p style="font-size:12px;color:#64748b;margin:0 0 20px;">${escapeHtml(data.period)}</p>

      <div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;padding:14px 16px;margin-bottom:20px;">
        <p style="font-size:13px;color:#0c4a6e;line-height:1.6;margin:0;">${escapeHtml(data.overview)}</p>
      </div>
